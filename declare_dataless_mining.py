...
def DeclareDataless_mining(log_path, filterCandidates=True, candidate_threshold=0.1, constraint_threshold=0.1,
    ...
    with open(output_file, 'a', encoding='UTF8') as f:
        ...
        if f.tell() == 0:
            header = ['log_filename', 'mining_algorithm', 'n_traces', 'log_trace_average_length', 'min_support', 'iteration_num',
        ...
        result = [log_path, 'ADM_S', len(log.traces), log.max_length, candidate_threshold, iteration_num, loading_time, mining_time]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--iterations', required=True, action='store',
                        type=int, dest='num_iters', default=False, help="provide number of iterations for the algorithm to run")
    args = parser.parse_args()

    for i in range(args.num_iters):
        DeclareDataless_mining(log_path=args.log_file, candidate_threshold=args.support, constraint_threshold=0, output_file=args.output_file, iteration_num=i)
