...
from wordy_confLexer import wordy_confLexer
from wordy_confParser import wordy_confParser
from wordy_confVisitor import wordy_confVisitor


def main(argv):
    input_stream = FileStream(argv[1])
    lexer = wordy_confLexer(input_stream)
    stream = CommonTokenStream(lexer)
    parser = wordy_confParser(stream)
    visitor = wordy_confVisitor()
    visitor.visitPipeline(parser.pipeline())
